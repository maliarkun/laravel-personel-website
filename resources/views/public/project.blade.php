        ...
        <article class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
            ...
            @if($project->featured_image)
                <img src="{{ asset('storage/' . $project->featured_image) }}" alt="{{ $project->title }}"
                    class="mt-6 w-full rounded-3xl border border-slate-800/60" />
            @endif
                ...
                <p class="mt-4 text-xs text-slate-400">
                    ...
                    @if($category)
                        <a href="{{ route('categories.show', $category) }}"
                            class="text-amber-200 hover:text-amber-100">{{ $category->name }}</a>
                    @else
                        <span>{{ __('project.uncategorized') }}</span>
                    @endif
                </p>
                <p class="mt-2 text-xs text-slate-400">{{ __('project.created_at') }}:

...
@section('admin-content')
    ...
    <form method="POST" action="{{ route('admin.notes.update', $note) }}" enctype="multipart/form-data"
        ...
        <div class="space-y-2">
            <label class="block text-sm uppercase tracking-[0.3em] text-slate-300">{{ __('notes.content') }}</label>
            <textarea name="content" id="editor"
                class="w-full rounded-3xl border border-slate-800 bg-slate-950/70 px-5 py-3 text-base text-slate-100 focus:border-amber-300 focus:outline-none">{{ old('content', $note->content) }}</textarea>
        </div>
            ...
            <div class="space-y-2 text-xs text-slate-400">
                ...
                <ul class="space-y-1">
                    @foreach($note->attachments as $attachment)
                        <li><a href="{{ $attachment->url }}" class="text-amber-200 hover:text-amber-100"
                                target="_blank">{{ $attachment->original_name }}</a></li>
                    @endforeach
                </ul>
            </div>
        @endif
